...
def get_input_data(result,img,x_res,y_res):
	...
	global className,truck_entry, truck_exit
	list1 = {}
	cyl = 0
	cyl_cap = 0
	cyl_count = 0
	...
	try:
		#print("helo1")
		#print(result)
		className_Truck = 'None'
		for i,j in enumerate(result):
			className_Truck = j[0]
			if className_Truck == 'No_truck':
				img, truck_entry, truck_exit = truck.truck_model(img,className_Truck)
			else:
				className_Truck = 'None'
		...
		for i,j in enumerate(result):
			...
			#if className == 'No_truck':
			#img = truck.truck_model(img,result)
			#print("No truck")
...
def top_model(img,darknet_image,network,class_names,truck_entry1):
	...
	try:
		...
		frame_rgb = cv2.cvtColor(img4, cv2.COLOR_BGR2RGB)
		...
		current_dict,current_len,img = get_input_data(result,img,x_res,y_res)
	...
	return img,total_cap_count, truck_entry, truck_exit
